	...
	<view class="page-container">
		...
		<view class="edit-area">
			...
			<view class="template-container">
				<image class="template-image" src="/static/moban2.jpg" mode="aspectFit"></image>
				...
				<view 
					...
					:style="{
						color: currentColor,
						fontFamily: getFontForText(currentText),
						fontSize: getPreviewFontSizePx() + 'px',
						fontWeight: 'bold',
						textAlign: 'center'
					}"
				>
					<text 
						v-for="(line, index) in getTextLines()" 
						:key="index" 
						class="text-line"
		...
		methods: {
			...
			getFontForText(text) {
				if (this.hasEnglishText(text)) {
					// 如果包含英文，使用常用英文字体（Arial, Helvetica等）
			...
			getPreviewFontSizePx() {
				// 当前容器宽按100vw渲染，等效放大倍数为 windowWidth / 400
				try {
					const sys = uni.getSystemInfoSync();
					const scale = sys.windowWidth / 400; // 与容器放大比例一致
			...
			measureTextWidth(text, fontSize, fontFamily) {
				...
				// 由于uni-app的限制，我们使用估算方法
				// 中文字符宽度约为字体大小的1倍，英文字符宽度约为字体大小的0.6倍
				let width = 0;
			...
			getTextLines() {
				...
				const maxWidth = 380; // 模板宽度400px，留20px边距
			...
			async autoMergeLayers() {
				...
				try {
					...
					ctx.font = `bold ${this.currentNameFontSize}px ${fontForText}`; // 设置粗体和字体
...
<style>
	...
	.center-text-preview {
		...
		top: 50%;
		left: 50%;
		transform: translate(-50%, calc(-50% + 5px)); /* 向下移动5px */
		pointer-events: none;
		user-select: none;
	...
	.text-line {
		...
		line-height: 1.2;
		white-space: nowrap;
		min-height: 1.2em; /* 确保空行也占用空间 */
	}
	
	.text-line.empty-line {
		opacity: 0; /* 空行不可见，但占用空间 */
	}
