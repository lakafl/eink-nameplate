	...
	<view class="page-container">
		...
		<view class="edit-area">
			...
			<view class="template-container">
				<image class="template-image" src="/static/moban3.jpg" mode="aspectFit"></image>
			...
			async autoMergeLayers() {
				...
				try {
					...
					try {
						...
						// 绘制单位文字（顶部20%区域居中位置，白色，可调节字体大小）
						if (this.currentUnit.trim()) {
							ctx.setFontSize(Math.round(this.currentUnitFontSize * scale));
							ctx.setFillStyle('#FFFFFF');
							ctx.setTextAlign('center');
							ctx.setTextBaseline('middle');
							ctx.font = `bold ${Math.round(this.currentUnitFontSize * scale)}px ${this.currentFont}`; // 设置粗体
							ctx.fillText(this.currentUnit, 200, 38); // 顶部20%区域居中位置
						}
						
						// 绘制姓名文字（底部80%区域居中位置，黑色，可调节字体大小）
						if (this.currentText.trim()) {
							ctx.setFontSize(Math.round(this.currentNameFontSize * scale));
							ctx.setFillStyle('#000000');
							ctx.setTextAlign('center');
							ctx.setTextBaseline('middle');
							ctx.font = `bold ${Math.round(this.currentNameFontSize * scale)}px ${this.currentFont}`; // 设置粗体
							ctx.fillText(this.currentText, 200, 175); // 底部80%区域居中位置
						}
					} catch (e) {
						// 兜底设置
						if (this.currentUnit.trim()) {
