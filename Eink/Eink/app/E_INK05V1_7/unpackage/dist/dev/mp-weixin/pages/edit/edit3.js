...
const _sfc_main = {
  ...
  methods: {
    ...
    adjustSendInterval(success) {
      if (success) {
        ...
        if (this.adaptiveTiming.successCount >= 3) {
          ...
          common_vendor.index.__f__("log", "at pages/edit/edit3.vue:808", `发送成功，缩短间隔时间至: ${this.adaptiveTiming.currentInterval}ms`);
        }
      } else {
        ...
        common_vendor.index.__f__("log", "at pages/edit/edit3.vue:819", `发送失败，增加间隔时间至: ${this.adaptiveTiming.currentInterval}ms`);
      }
    },
    // 重置自适应参数
    resetAdaptiveTiming() {
    ...
    async requestMTU(mtu) {
      try {
        common_vendor.index.__f__("log", "at pages/edit/edit3.vue:905", `尝试申请MTU到${mtu}字节`);
        if (!this.currentDeviceId) {
        ...
        common_vendor.index.__f__("log", "at pages/edit/edit3.vue:931", `MTU申请成功，实际MTU: ${actualMTU}字节`);
        const maxDataLength = actualMTU - 6;
        common_vendor.index.__f__("log", "at pages/edit/edit3.vue:935", `可用数据长度: ${maxDataLength}字节`);
        return maxDataLength;
      } catch (error) {
        const maxDataLength = 506 - 6;
        common_vendor.index.__f__("log", "at pages/edit/edit3.vue:943", `使用默认MTU: ${maxDataLength}字节`);
        return maxDataLength;
      }
    },
    // 发送数组数据
    async sendArrayData(dataArray, dataType) {
      ...
      common_vendor.index.__f__("log", "at pages/edit/edit3.vue:956", `发送${dataType === 37 ? "黑白" : "红白"}数组，共${totalPackets}个包，每包${maxDataLength}字节`);
      common_vendor.index.__f__("log", "at pages/edit/edit3.vue:957", `数组总长度: ${totalLength}字节`);
      ...
      for (let i = 0; i < totalPackets; i++) {
        ...
        common_vendor.index.__f__("log", "at pages/edit/edit3.vue:968", `发送第${i + 1}/${totalPackets}包，数据长度: ${packetData.length}字节`);
        const packet = this.buildDataPacket(packetData, dataType, false);
        await this.sendPacketWithQueue(packet);
        common_vendor.index.__f__("log", "at pages/edit/edit3.vue:975", `第${i + 1}包发送成功`);
      ...
      common_vendor.index.__f__("log", "at pages/edit/edit3.vue:988", `${dataType === 37 ? "黑白" : "红白"}数组发送完成，共发送${totalPackets}个包`);
    },
    // 构建数据包
    buildDataPacket(data, dataType, isLastPacket) {
      ...
      packet[index++] = 99;
      common_vendor.index.__f__("log", "at pages/edit/edit3.vue:1024", `构建数据包: 长度=${packet.length}, 数据类型=0x${dataType.toString(16)}, 数据长度=${dataLength}, 尾包=${isLastPacket}`);
      return packet;
    },
    // 发送尾包
    ...
    async sendPacket(packet, retryCount = 0) {
      ...
      try {
        ...
        common_vendor.index.__f__("log", "at pages/edit/edit3.vue:1154", `使用可写特征值: ${characteristic.uuid}`);
        ...
        common_vendor.index.__f__("log", "at pages/edit/edit3.vue:1162", `使用写入方式: ${useWriteNoResponse ? "writeNoResponse" : "write"}`);
        ...
        common_vendor.index.__f__("log", "at pages/edit/edit3.vue:1203", `发送数据包成功，长度: ${packet.length}字节`);
      } catch (error) {
        common_vendor.index.__f__("log", "at pages/edit/edit3.vue:1206", `发送数据包失败 (尝试 ${retryCount + 1}/${maxRetries + 1}):`, error);
        if (retryCount < maxRetries && (error.message.includes("写入失败") || error.message.includes("write") || error.message.includes("characteristic"))) {
          ...
          common_vendor.index.__f__("log", "at pages/edit/edit3.vue:1219", `等待 ${retryDelay}ms 后重试...`);
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    ...
    k: $data.currentFont,
    l: $options.getNamePreviewFontSizePx() + "px",
    m: common_vendor.o([($event) => $data.currentUnit = $event.detail.value, (...args) => $options.updateCurrentUnit && $options.updateCurrentUnit(...args)]),
    n: $data.currentUnit,
    o: common_vendor.o([($event) => $data.currentText = $event.detail.value, (...args) => $options.updateCurrentText && $options.updateCurrentText(...args)]),
    p: $data.currentText,
    q: $data.currentUnitFontSize,
    r: common_vendor.o((...args) => $options.onUnitFontSizeChange && $options.onUnitFontSizeChange(...args)),
    s: common_vendor.t($data.currentUnitFontSize),
    t: $data.currentNameFontSize,
    v: common_vendor.o((...args) => $options.onNameFontSizeChange && $options.onNameFontSizeChange(...args)),
    w: common_vendor.t($data.currentNameFontSize),
    x: $data.processedData.processing
  }, $data.processedData.processing ? {} : {}, {
    y: $data.sendingData
  }, $data.sendingData ? {
    z: $data.sendProgress + "%",
    A: common_vendor.t(Math.round($data.sendProgress))
  } : {}, {
    B: common_vendor.t($data.sendingData ? "发送中..." : "发送数据"),
    C: common_vendor.o((...args) => $options.sendDataToDevice && $options.sendDataToDevice(...args)),
    D: !$data.processedData.blackWhiteArray || !$data.processedData.redWhiteArray || !$data.isBluetoothConnected || $data.sendingData
  });
}
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["render", _sfc_render]]);
wx.createPage(MiniProgramPage);
//# sourceMappingURL=../../../.sourcemap/mp-weixin/pages/edit/edit3.js.map
